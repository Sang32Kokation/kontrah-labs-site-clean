...
export default function Home() {
  ...
  const features = [
    { icon: <LineChart className="h-6 w-6" />, title: "Performance First", desc: "Full-funnel media with ruthless clarity on ROI — Meta, Google, and more." },
    { icon: <Layers className="h-6 w-6" />, title: "Creative Intelligence", desc: "UGC, statics, videos tested methodically. Hooks, hold-rates, and story beats." },
    { icon: <Gauge className="h-6 w-6" />, title: "Signal Building", desc: "Cold-start playbooks that compound pixel learnings and cut CAC." },
    { icon: <ShieldCheck className="h-6 w-6" />, title: "Ownership Culture", desc: "Senior-level thinking, zero fluff. We treat your brand like our own." },
  ];

  const steps = [
    { num: "01", title: "Audit & Hypotheses", desc: "GA4 + ad account teardown. Identify constraints, quick wins, and a test backlog." },
      ...
      <section id="contact" className="py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="grid gap-8 md:grid-cols-5">
            ...
            <form
              className="md:col-span-3 rounded-3xl border border-white/10 bg-neutral-950 p-8"
              onSubmit={(e)=>{e.preventDefault(); alert('Form submitted. Replace with Web3Forms/Formspree.');}}
            >
...
function CaseCard({ tag, title, stat, sub }: { tag: string; title: string; stat: string; sub: string; }) {
  return (
